...
import { View, Text, TextInput, ScrollView, Platform, findNodeHandle } from 'react-native';
...
const TimerButtons = [
    {
        text: "Start Timer",
        onPress: () => DevRev.startTimer("test-event", { test_key1: "test-value1" })
    },
    {
        text: "Stop Timer",
...
const SessionAnalyticsScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
    ...
    try {
        ...
        DevRev.trackScreen("SessionAnalytics");
    } catch (error) {
    ...
    return (
        <ScrollView style={commonStyles.scrollView} contentContainerStyle={commonStyles.scrollViewContent}>
            <View style={commonStyles.container}>
                ...
                {Platform.OS === 'android' && (
                    <>
                        <Text style={commonStyles.heading}>Navigate to the Delayed Screen</Text>
                        <TouchableOpacityButton
